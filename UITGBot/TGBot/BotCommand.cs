        ...
        public static async Task<bool> SendMessage(string message, bool replyPrivateMessages, ITelegramBotClient client, Telegram.Bot.Types.Update update, CancellationToken token, bool replyMessage = false)
        {
            ...
            try
            {
                ...
                if (replyPrivateMessages)
                {
                    ...
                    UILogger.AddLog($"Сообщение отправлено ПОЛЬЗОВАТЕЛЮ {update.Message.From.Username} ({update.Message.From.Id})", "EXECUTION RESULT");
                }
                else
                {
                    if (replyMessage)
                    ...
                    UILogger.AddLog($"Сообщение отправлено В ПУБЛИЧНЫЙ ЧАТ {update.Message.Chat.Username} ({update.Message.Chat.Id})", "EXECUTION RESULT");
                }
            }
            catch (Exception e)
            {
